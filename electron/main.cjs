...
function bankInstructions(profile = 'standard') {
...
function ensureBankInstructions(profile) {
  const rulesDirectory = path.join(app.getPath('userData'), 'rules')
  const rulesPath = path.join(rulesDirectory, 'bank-coding.md')
  fs.mkdirSync(rulesDirectory, { recursive: true })
  fs.writeFileSync(rulesPath, `${bankInstructions(profile)}\n`, { mode: 0o600 })
  return rulesPath
}
...
function customProviderConfig(provider, settings) {
  const config = {
    ...
    instructions: [ensureBankInstructions(settings.accessMode === 'full' ? 'full' : settings.accessMode === 'plan' ? 'plan' : settings.accessMode === 'confirm' ? 'confirm' : settings.complianceProfile || 'standard')],
    share: 'disabled',
    autoupdate: false,
    compaction: {
