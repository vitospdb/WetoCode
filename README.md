...
WetoCode 是面向银行业研发团队的中文桌面 Coding Agent。它以 OpenCode 为本地执行引擎，在桌面端提供中文任务工作台、银行业安全规则、多模型接入、长上下文治理和自动更新。

Windows 安装包和免安装版请从 [GitHub Releases](https://github.com/vitospdb/WetoCode/releases) 下载。

当前仓库包含可运行的 `0.2.3` CLI 与桌面应用，不是静态界面稿。
...
- 集成终端默认进入安装包内置 WetoCode CLI，可切换项目 Shell；支持真实 PTY、Git 变更审阅/丢弃/检查点和隔离 Worktree
- 项目文件、上传图片和剪贴板图片附件
...
- 银行业规则注入：敏感信息、金额精度、幂等、审计、迁移和回滚约束
...
```bash
npm run pack:cli
npm install -g ./release/wetocode-0.2.3.tgz
wetocode --version
```

CLI 会复用 OpenCode 的 TUI、模型和会话能力，并注入与桌面版相同的中文银行研发规则。可通过 `wetocode providers` 配置认证，或使用 `OPENCODE_BIN` 指向企业审批过的引擎。
...
每次运行通过 `OPENCODE_CONFIG_CONTENT` 传入临时配置；银行规则文件存放在 WetoCode 的用户数据目录，不修改用户项目中的 `opencode.json`。会话分享默认关闭。
...
WetoCode 是研发辅助工具，不等于银行生产变更审批系统。当前桌面版保护的是本机使用边界：
...
1. 增加银行研发技能包：核心账务、支付清算、信贷风控、监管报送、数据仓库。
