...
当前仓库包含可运行的 `0.2.14` CLI 与桌面应用，不是静态界面稿。

## 草莓梦境主题

在“模型与设置”中选择主题卡片即可一键切换。草莓梦境将背景、玻璃面板、蕾丝顶边、控件与终端配色作为完整主题应用，无需手工修改配置文件。

![WetoCode 草莓梦境主题预览](./docs/images/strawberry-dream-preview.png)
...
- 集成终端默认进入连接本地执行服务的真实交互 TUI，以 WetoCode 品牌和中文固定界面文案显示；支持中文输入法、右键及快捷键复制粘贴，可切换项目 Shell；支持真实 PTY、Git 变更审阅/丢弃/检查点和隔离 Worktree
- 项目文件、上传图片和剪贴板图片附件
...
```bash
npm run pack:cli
npm install -g ./release/wetocode-0.2.14.tgz
wetocode --version
```
