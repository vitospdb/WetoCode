...
当前仓库包含可运行的 `0.2.7` CLI 与桌面应用，不是静态界面稿。
...
- 集成终端默认进入连接本地执行服务的真实交互 TUI，以 WetoCode 品牌显示，可切换项目 Shell；支持真实 PTY、Git 变更审阅/丢弃/检查点和隔离 Worktree
- 项目文件、上传图片和剪贴板图片附件
...
```bash
npm run pack:cli
npm install -g ./release/wetocode-0.2.7.tgz
wetocode --version
```
